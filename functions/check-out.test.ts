// check_out function-backed handler — TDD coverage.
//
// Mirrors check-in.test.ts: loads the REAL active ontology (which carries the
// hostel Booking/Guest types + the check_out action wired to
// functions/check-out.ts) and invokes through invokeAction — the same code path
...
//   - a checked-in booking → Booking.status becomes "completed"
//   - the booking's Guest.current_status becomes "checked_out"
...
// Guest, IncidentLog, plus the check_in / check_out / log_incident actions.
...
describe("check_out — active ontology wiring", () => {
  it("the yaml + function file discover one another (function: check-out, always_confirm)", () => {
    const def = ontology.action_types.check_out;
    expect(def).toBeDefined();
    expect(def?.function).toBe("check-out");
    expect(def?.agent_policy).toBe("always_confirm");
    expect(def?.permissions).toEqual(["steward", "manager"]);
  });
});

describe("check_out — handler effect", () => {
  it("completes the booking and mirrors the guest's current_status to checked_out", async () => {
    ...
    expect(guest?.current_status).toBe("checked_out");
  });
  ...
  it("DENIES a member-actor invocation (permission inherited from dispatcher) and mutates nothing", async () => {
    ...
    ).rejects.toThrow(/cannot invoke action "check_out"/);
