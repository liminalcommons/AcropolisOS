...
4. **`MeetingMinute` board widget on long-lived instances — RESOLVED.** `MeetingMinute` is a **platform object type** (`lib/ontology/ctx.ts` declares it present in every ontology), so the board auto-composes a count widget for it. On a **fresh clone** `drizzle-kit push` creates its table along with all others, so the widget renders fine (`0 count`). But on an instance whose DB predates the type's addition to the platform contract, `push` **silently skips the new table on non-TTY stdin** (the same documented behaviour that already affects `approved_views`/`channel_bindings`), so the widget's `COUNT(*)` hit a nonexistent relation and surfaced as "could not be loaded." **Fix:** `docker-entrypoint.sh` now ensures `meeting_minute` with an idempotent `CREATE TABLE IF NOT EXISTS` *after* push (so the FK target `event` exists) and verifies it via `check_column` — a no-op on fresh installs, a self-heal on long-lived ones. Verified on the live dogfood instance: the widget now renders `0 count` instead of erroring.

> **Note on live vs. report KPI values.** The canonical KPIs above (autonomy 73% · acceptance 100% · coverage 94% · accuracy 93%) are the **deterministic fresh-clone** values from the documented repro sequence. A long-lived dogfood instance (e.g. the `Riverside Collective` demo) will show slightly different numbers (e.g. coverage 100% / accuracy 95%) because — by design (Gap #1) — each KPI tracks **that instance's own accumulated decision history**, not a fixed fixture. Both are correct; only the fresh-clone values are reproducible by a reviewer.
